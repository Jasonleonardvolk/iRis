...
export async function initializeUserMemory(uid: string): Promise<boolean> {
  if (!uid) {
    console.error('🌊 Soliton Memory Error: Cannot initialize with empty userId');
    return false;
  }

  userId = uid;
  console.log(`🌊 Soliton Memory: Initializing for user ${userId}`);

  try {
    // Initialize on backend first
    const requestBody = { userId: uid };
    console.log('🌊 Soliton Memory: Sending init request:', requestBody);
    ...
    if (!response.ok) {
      ...
      console.error('🌊 Soliton Memory: Init failed:', response.status, errorText);
    ...
    if (result.success) {
      console.log('🌊 Soliton Memory: Backend initialization successful');
      ...
      fetchMemoryStats(uid).catch(err => {
        console.warn('🌊 Soliton Memory: Stats fetch failed during init:', err);
      });
  ...
  const msg = error instanceof Error ? error.message : String(error);
    console.error('🌊 Soliton Memory Error:', error);
    
    // Only use fallback if explicitly enabled in environment
    if (CONFIG.ALLOW_FALLBACK && browser) {
      console.warn('🌊 Soliton Memory: Using fallback mode (LOCAL ONLY - NO PERSISTENCE)');
      loadFromLocalStorage();
      isInitialized = true;
      return true;
...
export async function storeMemory(
  ...
  if (!isInitialized || !userId) {
    console.error('🌊 Soliton Memory Error: Not initialized. Call initializeUserMemory first.');
    return false;
  }
  ...
  const msg = error instanceof Error ? error.message : String(error);
    ...
    if (CONFIG.ALLOW_FALLBACK && browser) {
      console.warn('🌊 Soliton Memory: Memory storage failed on backend, using local only');
      addToLocalCache(memoryEntry);
      saveToLocalStorage();
      return true;
...
export async function findRelatedMemories(
  ...
  if (!isInitialized || !userId) {
    console.error('🌊 Soliton Memory Error: Not initialized');
    return [];
  }
  ...
  const msg = error instanceof Error ? error.message : String(error);
    ...
    // Only use fallback if explicitly enabled
    if (CONFIG.ALLOW_FALLBACK && browser) {
      console.warn('🌊 Soliton Memory: Recall from backend failed, using local cache');
...
export async function fetchMemoryStats(uid: string): Promise<MemoryStats | null> {
  ...
  if (lastStatsFailure > 0 && now - lastStatsFailure < STATS_RETRY_COOLDOWN) {
    console.log('🌊 Soliton Memory: Skipping stats fetch due to recent failure');
  ...
  console.log('🌊 Soliton Memory: Fetching stats for slug:', userSlug);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorText = await response.text();
      console.error('🌊 Soliton Memory: Stats failed:', response.status, errorText);
  ...
  const msg = error instanceof Error ? error.message : String(error);
    ...
    // Only use fallback if explicitly enabled
    if (CONFIG.ALLOW_FALLBACK && browser) {
      console.warn('🌊 Soliton Memory: Stats fetch from backend failed, using local estimates');
...
export async function listMemoriesByPrefix(prefix: string): Promise<MemoryEntry[]> {
  ...
  const msg = error instanceof Error ? error.message : String(error);
    ...
    // Only use fallback if explicitly enabled
    if (CONFIG.ALLOW_FALLBACK && browser) {
      console.warn('🌊 Soliton Memory: List-by-prefix from backend failed, using local cache');
...
export async function phaseChangeEvent(event: PhaseChangeEvent): Promise<boolean> {
  ...
  try {
    ...
    if (result.success) {
      console.log(`🌊 Ghost AI Phase Change: ${event.phase} (amplitude: ${event.amplitude.toFixed(2)}, frequency: ${event.frequency.toFixed(2)})`);
      return true;
    } else {
  ...
  const msg = error instanceof Error ? error.message : String(error);
    ...
    console.warn('🌊 Ghost AI Phase Change (local only):', event.phase);
    return false;
...
async function generateEmbedding(text: string): Promise<number[]> {
  ...
  const msg = error instanceof Error ? error.message : String(error);
    console.warn('🌊 Soliton Memory: Embedding generation on backend failed, using fallback method');
  ...
  try {
    ...
    if (storedData) {
      ...
      if (Array.isArray(parsed)) {
        ...
        console.log(`🌊 Soliton Memory: Loaded ${localMemoryCache.length} memories from localStorage`);
      }
    }
  } catch (error) {
  const msg = error instanceof Error ? error.message : String(error);
    console.error('🌊 Soliton Memory: Error loading from localStorage', error);
...
function saveToLocalStorage(): void {
  ...
  const msg = error instanceof Error ? error.message : String(error);
    console.error('🌊 Soliton Memory: Error saving to localStorage', error);

...
// Define WavefieldParameters interface locally
...
export class PhotoMorphPipeline {
  ...
  // Add definite assignment assertions (!) for properties initialized in initialize()
  ...
  private async propagateWavefield(
    ...
    // Create output texture - Use width/height instead of size
    ...
    this.device.queue.writeBuffer(propBuffer, 0, propParams.buffer);
    ...
    // Use width/height instead of size[0]/size[1]
  ...
  private async generateVelocityField(
    ...
    // Use width/height instead of size
    ...
    const wavefieldParams = new Float32Array([
      ...
      ...Array.from(this.generateSpatialFrequencies(oscillatorPhases))
    ]);
  ...
  // Implement missing conversion methods
  ...
  // Add missing createLenticularParams method
